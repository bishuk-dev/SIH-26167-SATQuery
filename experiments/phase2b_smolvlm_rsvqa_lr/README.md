...
Training and hyperparameter decisions use only train and validation. The test split is evaluated once the configuration is frozen; the configured 100-question comparison samples test questions round-robin across scenes. Images remain under ignored `data/benchmarks/rsvqa_lr_phase2b/`; only the provenance and split manifest is versioned.
