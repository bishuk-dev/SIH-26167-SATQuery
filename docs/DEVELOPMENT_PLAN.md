...
**Phase 3B/final grounding validation status: complete and frozen; one-time test pending.** Threshold 0.30 plus an exclusive normalized-area cap of 0.80 is the frozen production policy. Boxes at or above the cap are discarded, the highest-scoring remaining model box is selected, and an empty remainder becomes valid abstention evidence. Deterministic replay on the 24 stored validation candidates produced mean IoU 0.2058, Acc@0.5 25.00%, 9 abstentions, 15 detections, detected-only mean IoU 0.3293, and zero oversized selections. Phase 3 validation tuning is closed. A fail-closed Kaggle evaluator is ready, but the VRSBench test split remains untouched because the local checkpoint download stalled before inference; Phase 3 becomes complete only after the two immutable final-test artifacts are returned and recorded without tuning.
